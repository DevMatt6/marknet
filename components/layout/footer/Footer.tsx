"use client";

import Link from "next/link";
import { motion } from "framer-motion";
import Logo from "@/components/common/Logo";
import ButtonDemo from "@/components/shadcn-space/button/button-16";
import { useMessages } from "@/providers/LocaleProvider";
import { siteConfig } from "@/config/site";

type SocialIconProps = { size?: number; strokeWidth?: number };

function InstagramIcon({ size = 16, strokeWidth = 1.5 }: SocialIconProps) {
	return (
		<svg
			xmlns="http://www.w3.org/2000/svg"
			width={size}
			height={size}
			viewBox="0 0 24 24"
			fill="none"
			stroke="currentColor"
			strokeWidth={strokeWidth}
			strokeLinecap="round"
			strokeLinejoin="round"
			aria-hidden="true"
		>
			<rect width="20" height="20" x="2" y="2" rx="5" ry="5" />
			<path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" />
			<line x1="17.5" x2="17.51" y1="6.5" y2="6.5" />
		</svg>
	);
}

function LinkedinIcon({ size = 16, strokeWidth = 1.5 }: SocialIconProps) {
	return (
		<svg
			xmlns="http://www.w3.org/2000/svg"
			width={size}
			height={size}
			viewBox="0 0 24 24"
			fill="none"
			stroke="currentColor"
			strokeWidth={strokeWidth}
			strokeLinecap="round"
			strokeLinejoin="round"
			aria-hidden="true"
		>
			<path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
			<rect width="4" height="12" x="2" y="9" />
			<circle cx="4" cy="4" r="2" />
		</svg>
	);
}

function FacebookIcon({ size = 16, strokeWidth = 1.5 }: SocialIconProps) {
	return (
		<svg
			xmlns="http://www.w3.org/2000/svg"
			width={size}
			height={size}
			viewBox="0 0 24 24"
			fill="none"
			stroke="currentColor"
			strokeWidth={strokeWidth}
			strokeLinecap="round"
			strokeLinejoin="round"
			aria-hidden="true"
		>
			<path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z" />
		</svg>
	);
}

function GithubIcon({ size = 16, strokeWidth = 1.5 }: SocialIconProps) {
	return (
		<svg
			xmlns="http://www.w3.org/2000/svg"
			width={size}
			height={size}
			viewBox="0 0 24 24"
			fill="none"
			stroke="currentColor"
			strokeWidth={strokeWidth}
			strokeLinecap="round"
			strokeLinejoin="round"
			aria-hidden="true"
		>
			<path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
			<path d="M9 18c-4.51 2-5-2-7-2" />
		</svg>
	);
}

const socialIcons = {
	instagram: InstagramIcon,
	linkedin: LinkedinIcon,
	facebook: FacebookIcon,
	github: GithubIcon,
};

export default function Footer() {
	const { footer } = useMessages();
	const year = new Date().getFullYear();

	const socials = Object.entries(siteConfig.social).filter(
		([, href]) => href && href.trim() !== "",
	) as [keyof typeof socialIcons, string][];

	return (
		<footer className="bg-bg-secondary">
			<div className="mx-auto w-full max-w-full px-5 sm:px-8 lg:px-10">
				{/* Main grid */}
				<motion.div
					className="grid grid-cols-1 gap-12 py-16 md:grid-cols-[1fr_auto_auto_auto] md:gap-16 lg:gap-24"
					initial={{ opacity: 0, y: 24 }}
					whileInView={{ opacity: 1, y: 0 }}
					viewport={{ once: true, amount: 0.15 }}
					transition={{ duration: 0.6 }}
				>
					{/* Brand column */}
					<div className="flex flex-col gap-5">
						<Link href="/" aria-label="Marknet">
							<Logo width={110} height={28} />
						</Link>
						<p className="font-body max-w-[22ch] text-sm leading-relaxed text-text-secondary">
							{footer.tagline}
						</p>
						{/* CTAs */}
						<div className="mt-2 flex flex-wrap gap-3">
							<ButtonDemo
								text={footer.cta.startProject}
								variant="default"
								size="sm"
							/>
							<ButtonDemo
								text={footer.cta.joinTeam}
								variant="outline"
								size="sm"
							/>
						</div>
						{/* Social icons */}
						{socials.length > 0 && (
							<div className="mt-2 flex items-center gap-4">
								{socials.map(([key, href]) => {
									const Icon = socialIcons[key];
									return (
										<a
											key={key}
											href={href}
											target="_blank"
											rel="noopener noreferrer"
											aria-label={key}
											className="text-text-secondary transition-colors duration-200 hover:text-text-primary"
										>
											<Icon size={16} strokeWidth={1.5} />
										</a>
									);
								})}
							</div>
						)}
					</div>

					{/* Navigation column */}
					<div className="flex flex-col gap-4">
						<p className="font-body text-xs font-semibold uppercase tracking-widest text-text-secondary">
							{footer.nav.label}
						</p>
						<nav className="flex flex-col gap-3">
							{footer.nav.items.map((item) => (
								<Link
									key={item.href + item.label}
									href={item.href}
									className="font-body text-sm text-text-primary transition-colors duration-200 hover:text-accent"
								>
									{item.label}
								</Link>
							))}
						</nav>
					</div>

					{/* Services column */}
					<div className="flex flex-col gap-4">
						<p className="font-body text-xs font-semibold uppercase tracking-widest text-text-secondary">
							{footer.services.label}
						</p>
						<nav className="flex flex-col gap-3">
							{footer.services.items.map((item) => (
								<Link
									key={item.href + item.label}
									href={item.href}
									className="font-body text-sm text-text-primary transition-colors duration-200 hover:text-accent"
								>
									{item.label}
								</Link>
							))}
						</nav>
					</div>

					{/* Contact column */}
					{siteConfig.contact.email && (
						<div className="flex flex-col gap-4">
							<p className="font-body text-xs font-semibold uppercase tracking-widest text-text-secondary">
								{footer.contact.label}
							</p>
							<div className="flex flex-col gap-3">
								<a
									href={`mailto:${siteConfig.contact.email}`}
									className="font-body text-sm text-text-primary transition-colors duration-200 hover:text-accent"
								>
									{siteConfig.contact.email}
								</a>
								{siteConfig.contact.phone && (
									<a
										href={`tel:${siteConfig.contact.phone}`}
										className="font-body text-sm text-text-primary transition-colors duration-200 hover:text-accent"
									>
										{siteConfig.contact.phone}
									</a>
								)}
							</div>
						</div>
					)}
				</motion.div>

				{/* Bottom bar */}
				<div className="flex flex-col items-start justify-between gap-4 border-t border-border py-6 sm:flex-row sm:items-center">
					<p className="font-body text-xs text-text-secondary">
						© {year} Marknet. {footer.copyright}
					</p>
					{/* Mobile social icons (when no contact column shown) */}
					{socials.length > 0 && !siteConfig.contact.email && (
						<div className="flex items-center gap-4">
							{socials.map(([key, href]) => {
								const Icon = socialIcons[key];
								return (
									<a
										key={key}
										href={href}
										target="_blank"
										rel="noopener noreferrer"
										aria-label={key}
										className="text-text-secondary transition-colors duration-200 hover:text-text-primary"
									>
										<Icon size={14} strokeWidth={1.5} />
									</a>
								);
							})}
						</div>
					)}
				</div>
			</div>
		</footer>
	);
}
